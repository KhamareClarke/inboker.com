...
export async function signUp(
  ...
  if (error) {
    ...
    if (
      ...
      (errorMsg.includes('request url') || errorMsg.includes('url'))
    ) {
      throw new Error(
        'Supabase URL is misconfigured. Use only your project base URL (for example https://xxxxx.supabase.co), not a path like /rest/v1. Fix NEXT_PUBLIC_SUPABASE_URL in .env.local and restart the dev server.'
      );
    }
