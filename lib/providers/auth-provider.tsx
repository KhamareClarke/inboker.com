...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  /** Idle session timeout (browser tab): sign out after inactivity. */
  useEffect(() => {
    if (!user || !supabase) return;
    const idleMs = Math.max(
      60_000,
      parseInt(process.env.NEXT_PUBLIC_SESSION_IDLE_MS || `${45 * 60 * 1000}`, 10) || 45 * 60 * 1000
    );
    ...
    evs.forEach((ev) => window.addEventListener(ev, reset, { passive: true } as AddEventListenerOptions));
    return () => {
      clearTimeout(timer);
