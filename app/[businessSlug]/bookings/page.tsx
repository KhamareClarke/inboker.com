  ...
  if (loading) {
    return (
      <div 
        ...
        style={{
          background: profile 
            ? `linear-gradient(135deg, ${profile.primary_color}15 0%, ${profile.secondary_color}15 100%)`
            : 'linear-gradient(135deg, #3b82f615 0%, #06b6d415 100%)'
        }}
      >
        <Loader2 
          className="h-8 w-8 animate-spin" 
          style={{ color: profile?.primary_color || '#3b82f6' }} 
        />
      </div>
    );
  }
      ...
      <Section>
        ...
        <div className="mb-ds-3">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <h1 className="ds-heading-2">{profile.business_name}</h1>
              <p className="ds-body-sm text-muted-foreground">Manage Booking Links</p>
            </div>
          </div>
        </div>
        ...
        {services.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-ds-2 sm:gap-ds-3">
            {services.map((service) => {
              const bookingUrl = getBookingUrl(service);
              const isCopied = copiedId === service.id;

              return (
                <Card key={service.id} className="rounded-ds-lg shadow-ds-md hover:shadow-ds-lg transition-shadow">
                  <CardHeader>
