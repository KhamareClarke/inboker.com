...
export default function BusinessDashboardPage() {
  ...
  // CRITICAL: Check subscription status - NO dashboard access without payment
  useEffect(() => {
    if (user && !authLoading) {
      const checkSubscription = async () => {
        try {
          ...
          if (response.ok && data.subscription) {
            ...
            // Only allow active, trialing, or trial status
            if (status !== 'active' && status !== 'trialing' && status !== 'trial') {
              // IMMEDIATELY redirect to billing - NO dashboard access
  ...
  const stats = useMemo(() => {
    ...
    return [
      { title: 'Total Bookings', value: totalCount.toString(), icon: BookOpen, description: `${upcomingCount} upcoming, ${completedCount} completed`, gradient: 'from-blue-600 to-cyan-600', bgGradient: 'from-blue-50 to-cyan-50' },
      { title: 'Active Clients', value: clientCount.toString(), icon: Users, description: `${clientCount} unique client${clientCount !== 1 ? 's' : ''}`, gradient: 'from-green-600 to-emerald-600', bgGradient: 'from-green-50 to-emerald-50' },
      { title: 'Upcoming', value: upcomingCount.toString(), icon: Calendar, description: `${upcomingCount} upcoming appointment${upcomingCount !== 1 ? 's' : ''}`, gradient: 'from-orange-600 to-amber-600', bgGradient: 'from-orange-50 to-amber-50' },
      { title: 'Completed', value: completedCount.toString(), icon: CheckCircle2, description: `${completedCount} completed appointment${completedCount !== 1 ? 's' : ''}`, gradient: 'from-purple-600 to-pink-600', bgGradient: 'from-purple-50 to-pink-50' },
      { title: 'Revenue', value: revenueText, icon: TrendingUp, description: 'Total from completed appointments', gradient: 'from-teal-600 to-cyan-600', bgGradient: 'from-teal-50 to-cyan-50' },
    ];
  ...
  const secondary = profile.secondary_color || '#06b6d4';

  return (
    <div className="min-h-screen" style={{ background: `linear-gradient(to bottom right, ${primary}08, ${secondary}08, #f8fafc)` }}>
      <div className="border-b" style={{ background: `linear-gradient(135deg, ${primary}12 0%, ${secondary}08 100%)`, borderColor: `${primary}20` }}>
            ...
            <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
              {profile.logo_url && (
                <img
                  ...
                  className="w-10 h-10 sm:w-12 sm:h-12 object-contain flex-shrink-0 rounded-lg bg-white shadow-sm"
                />
              )}
              <div className="min-w-0">
                <h1 className="font-display text-xl sm:text-2xl font-bold text-gray-900 truncate">{profile.business_name}</h1>
                <p className="text-xs sm:text-sm text-gray-500">Welcome back, {userName}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              <InboxBell className="h-9 w-9 border bg-background/95 text-foreground shadow-sm shrink-0" />
              <Button
                size="sm"
                variant="outline"
                onClick={() => router.push(`/${businessSlug}/dashboard/analytics`)}
                className="hidden sm:flex bg-background/90"
              >
                Analytics
              </Button>
              <Button
              ...
              <Button
                ...
                className="hidden sm:flex text-white border-0"
      ...
      <Section className="py-ds-4 sm:py-ds-5 lg:py-ds-6">
        <Container size="wide">
        <Card className="mb-6 rounded-2xl border-2 shadow-md">
          <CardHeader className="rounded-t-2xl text-white" style={{ background: `linear-gradient(135deg, ${primary} 0%, ${secondary} 100%)` }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <Card
            className="border-2 hover:shadow-xl transition-all duration-300 cursor-pointer rounded-2xl shadow-md"
            onClick={() => router.push(`/${businessSlug}/services`)}
          >
            <CardHeader className="bg-gradient-to-r from-purple-50 via-pink-50 to-rose-50 border-b rounded-t-2xl">
          ...
          >
            <CardHeader className="bg-gradient-to-r from-orange-50 via-amber-50 to-yellow-50 border-b rounded-t-2xl">
              <div className="flex items-center justify-between">
        ...
        <Card className="border-2 rounded-2xl shadow-md">
          <CardHeader className="bg-gradient-to-r from-slate-50 to-gray-50 border-b rounded-t-2xl">
            <CardTitle>Quick Actions</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Button
                className="h-20 flex flex-col items-center justify-center bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white border-0"
              ...
              <Button
                className="h-20 flex flex-col items-center justify-center bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-0"
              ...
              <Button
                className="h-20 flex flex-col items-center justify-center bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white border-0"
                onClick={() => router.push('/dashboard/brand')}
              >
