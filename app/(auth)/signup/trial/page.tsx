...
export default function TrialSignupPage() {
  ...
  if (authLoading) {
    return (
      <Section as="div" className="flex items-center justify-center min-h-screen py-ds-6">
      ...
      <Container size="narrow" className="max-w-2xl w-full relative z-10">
        <Card className="w-full border-2 shadow-ds-xl backdrop-blur-sm bg-background/80 rounded-ds-lg">
          <CardHeader className="space-y-3 pb-6 text-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-r from-blue-600 to-cyan-600 flex items-center justify-center mx-auto mb-ds-2 shadow-ds-md">
              <Gift className="h-10 w-10 text-white" />
            </div>
            <CardTitle className="ds-heading-2">Start Your 14-Day Free Trial</CardTitle>
            <CardDescription className="ds-body text-lg">
        ...
        <CardContent className="space-y-ds-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-ds-2">
            <Card 
              className={`cursor-pointer transition-all rounded-ds-lg ${selectedPlan === 'monthly' ? 'border-blue-500 border-2 shadow-ds-lg' : 'border'}`}
              onClick={() => setSelectedPlan('monthly')}
            >
              <CardHeader>
                <CardTitle className="ds-heading-3">{monthlyPlan.name}</CardTitle>
                <div className="mt-2">
            ...
            <Card 
              className={`cursor-pointer transition-all rounded-ds-lg ${selectedPlan === 'annually' ? 'border-blue-500 border-2 shadow-ds-lg' : 'border'}`}
              onClick={() => setSelectedPlan('annually')}
            >
              <CardHeader>
                <CardTitle className="ds-heading-3">{annualPlan.name}</CardTitle>
                <div className="mt-2">
          ...
          <div className="flex items-start gap-ds-2 p-ds-2 bg-muted rounded-ds">
            <input
              type="checkbox"
              id="agree"
              checked={agreedToTerms}
              onChange={(e) => setAgreedToTerms(e.target.checked)}
              className="mt-1 h-4 w-4 rounded-ds border-gray-300"
            />
            <label htmlFor="agree" className="ds-body-sm text-muted-foreground cursor-pointer">
