...
import { Loader2, Users, Building2, Calendar, Star, Trash2, AlertTriangle, LogOut, Shield, TrendingUp, KeyRound, UserCog } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  const [resetResultDialog, setResetResultDialog] = useState<{
    open: boolean;
    email: string;
  ...
  const handleAdminResetPassword = async (u: {
    ...
    try {
      const res = await postAdminResetPassword(supabase, { userId: u.id });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data.error || 'Reset failed');
      }
      setResetResultDialog({
        open: true,
        email: u.email,
      ...
      toast({
        ...
        description: data.emailSent
          ? 'Notice emailed (no password in email). Copy the temporary password from the dialog.'
          : 'Email notice was not sent. Copy the temporary password from the dialog.',
      });
    } catch (err: any) {
      toast({
        variant: 'destructive',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2 justify-end">
                        <Button
                          ...
                          disabled={owner.suspended || resettingUserId === owner.id}
                          onClick={() => void handleAdminResetPassword(owner)}
                        >
                          {resettingUserId === owner.id ? (
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2 justify-end">
                        <Button
                          ...
                          size="sm"
                          className="border-purple-500/30"
                          disabled={customer.suspended || resettingUserId === customer.id}
                          onClick={() => void handleAdminResetPassword(customer)}
                        >
      ...
      <Dialog
        open={Boolean(resetResultDialog?.open)}
        onOpenChange={(open) => {
          if (!open) setResetResultDialog(null);
        }}
      >
        <DialogContent className="bg-slate-800 border-purple-500/20 sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400">
              Share this with {resetResultDialog?.email} securely. It is not sent by email.
            </DialogDescription>
          </DialogHeader>
          {resetResultDialog && (
            <div className="space-y-3">
              <div className="rounded-ds bg-slate-900/80 border border-purple-500/30 p-3 font-mono text-sm text-white break-all">
                {resetResultDialog.temporaryPassword}
              </div>
              <p className="text-xs text-gray-500">
                Email notice sent: {resetResultDialog.emailSent ? 'yes' : 'no (check SMTP env)'}
              </p>
            </div>
          )}
          <DialogFooter className="gap-2 sm:gap-0">
            <Button
              ...
              className="border-purple-500/20"
              onClick={() => {
                if (resetResultDialog?.temporaryPassword) {
                  void navigator.clipboard.writeText(resetResultDialog.temporaryPassword);
            ...
            <Button
              onClick={() => setResetResultDialog(null)}
